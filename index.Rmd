...
<center>Total Confirmed Cases - Excluding China</center>
...
#Download shapefile
 #download.file('http://biogeo.ucdavis.edu/data/diva/adm/IRN_adm.zip', destfile='shapefile/iran_province_shapefile.zip')
 #unzip('shapefile/iran_province_shapefile.zip', exdir='shapefile')
 #file.remove('shapefile/iran_province_shapefile.zip')
...
ggplot(data = merged_data) +
  geom_polygon(aes(x=long, y=lat, group=group, fill=Total_cases, text=paste(Province, ' >> ', Total_cases)),
               ...
               size = 0.1) +
  coord_fixed(1.3) +
  scale_fill_gradient(high = "purple", low = "white", guide = "colorbar", breaks=pretty_breaks(n=5)) +
  ...
  labs(title = "") 
ggplotly(map, tooltip=c("x", "y", "text"))
...
df1 %>% 
  ...
  DT::datatable(rownames = FALSE,
                ...
                colnames = c('Date', 'Country', 'Province.State', 'Confirmed', 'Recovered', 'Death'),
                filter   = 'top')
```
...
#file.remove('shapefile/iran_province_shapefile.zip')
library(sf)
sf <- st_read("shapefile/IRN_adm1.shp")
library(ggplot2)
ggplot() + 
  geom_sf(data = sf, size = 0.02, color = "black", fill = "pink") + 
  ggtitle("Hadi Test") + 
  coord_sf()
